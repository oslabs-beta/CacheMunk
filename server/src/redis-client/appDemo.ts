...
const cache = CacheMunk(redis, {
  onCacheHit: ((queryKey, executionTime, dataLengthBytes) => {
    console.log(`cache hit on query ${queryKey} (${dataLengthBytes.toString()} bytes) in ${executionTime.toString()} ms. `);
  }),
  ...
  onWrite: ((queryKey, executionTime, originalSize, compressedSize) => {
    console.log(`wrote: ${compressedSize.toString()} bytes to cache at ${queryKey} in ${executionTime.toString()} ms`);
